...
export default function TopBets() {
  ...
  useEffect(() => {
    if (activeTournament) {
      const events = topBets?.events.filter(
        (item) => item.sport_tournament_id === activeTournament
      );
      setFixtures(events);
    }
  ...
  return (
    <div className="topbets-container">
      ...
      <div className="topbets-bottom">
        {topBets &&
          topBets.topbets.map((topbet) => (
            <div
              onClick={() => setActiveTournament(topbet?.tournament_id)}
              className={`topbets-cell sg_1 ${
                topbet?.tournament_id === activeTournament ? "selected" : ""
              }`}
              key={`topbet-item-${topbet.id}`}
            >
              <p className="nomeGruppo ng-binding">
                {topbet.tournament.category.name}
              </p>
              <h4 className="nomeEvento ng-binding">
                {topbet.tournament.name}
              </h4>
            </div>
          ))}
      </div>
                  ...
                  {fixture.events.map((match) => (
                    <>
                      ...
                      <tr key={`c_${match.id}`}>
                        <td colSpan="11">
                          <div
                            className={`SingleInsideStats_${match.provider_id}`}
                          />
                        </td>
                      </tr>
                    </>
